...
export const Overview24hChart: React.FC<Overview24hChartProps> = ({ inverterId, arrayId, date }) => {
  ...
  const fetchOverviewData = async () => {
    try {
      ...
      if (!arrayId && !inverterId) {
        console.warn('Overview24hChart: No arrayId or inverterId provided, using mock data');
      ...
      // Fetch from /api/energy/hourly or /api/arrays/{arrayId}/energy/hourly
      try {
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow p-4 sm:p-6">
        <h3 className="text-sm sm:text-lg font-semibold mb-4">Overview</h3>
        <div className="h-48 sm:h-64 flex items-center justify-center">
          <div className="text-gray-500 text-xs sm:text-sm">Loading overview data...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
        <h3 className="text-sm sm:text-lg font-semibold">24-Hour Overview</h3>
