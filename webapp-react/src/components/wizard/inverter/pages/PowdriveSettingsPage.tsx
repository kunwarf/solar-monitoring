...
import { PowdriveTOUCard } from '../sections/PowdriveTOUCard'

interface PowdriveSettingsPageProps {
  inverterId: string
}

export const PowdriveSettingsPage: React.FC<PowdriveSettingsPageProps> = ({
  inverterId
}) => {
    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold mb-2" style={{ color: textColor }}>
          Powdrive Inverter Settings
        </h2>
        <p className="text-sm" style={{ color: textSecondary }}>
          Configure Powdrive inverter settings and controls
        </p>
      </div>
      ...
      <PowdriveTOUCard inverterId={inverterId} batteryOperation={batteryType?.battery_operation} />
    </div>
  )
}
