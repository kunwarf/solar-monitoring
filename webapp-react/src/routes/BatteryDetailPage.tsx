...
const ProgressBar: React.FC<{ 
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <span className="text-base font-medium min-w-[60px]" style={{ color: textColor }}>{displayValue} {unit}</span>
    </div>
  )
}

export const BatteryDetailPage: React.FC = () => {
  const navigate = useNavigate()
  const { isMobile, isCompact } = useMobile()
  const { theme } = useTheme()
  const [bank, setBank] = useState<BatteryBank | null>(null)
  const [allBanks, setAllBanks] = useState<BatteryBank[]>([])
  ...
  const fetchBattery = async () => {
    try {
      ...
      const res = await api.get('/api/battery/now?bank_id=all') as any
      if (res && res.status === 'ok') {
        const activeBanks = res.banks || (res.battery ? [res.battery] : [])
        const configuredBanks = res.configured_banks || []
        
        const allBanksList: BatteryBank[] = []
        
        for (const configBank of configuredBanks) {
          const activeBank = activeBanks.find((b: BatteryBank) => b.id === configBank.id)
          if (activeBank) {
            // Add name from config to active bank
            allBanksList.push({
              ...activeBank,
              name: configBank.name || activeBank.name || configBank.id
            })
          } else {
            allBanksList.push({
              ts: new Date().toISOString(),
              id: configBank.id,
              name: configBank.name || configBank.id,
              batteries_count: 0,
              cells_per_battery: 0,
              devices: [],
            } as BatteryBank)
        ...
        if (allBanksList.length === 0 && activeBanks.length > 0) {
          // If no configured banks, use active banks and try to get names from config
          for (const activeBank of activeBanks) {
            const configBank = configuredBanks.find((cb: any) => cb.id === activeBank.id)
            allBanksList.push({
              ...activeBank,
              name: configBank?.name || activeBank.name || activeBank.id
            })
          }
        }
        
        if (allBanksList.length > 0) {
          setAllBanks(allBanksList)
          
          if (!selectedBankId || !allBanksList.find(b => b.id === selectedBankId)) {
            setSelectedBankId(allBanksList[0].id)
          }
          
          const selected = allBanksList.find(b => b.id === selectedBankId) || allBanksList[0]
          setBank(selected)
        setError(null)
        } else {
  ...
  useEffect(() => {
    fetchBattery()
    const t = setInterval(fetchBattery, 5000)
    return () => clearInterval(t)
  }, [selectedBankId])

  useEffect(() => {
    if (selectedBankId) {
      const selected = allBanks.find((b: BatteryBank) => b.id === selectedBankId)
      if (selected) {
        setBank(selected)
        setError(null)
      }
    }
  }, [selectedBankId, allBanks])
    ...
    return (
      <>
        ...
        {(isCharging || isDischarging) && (
          <div 
            className={`absolute ${isMobile ? 'top-2 right-2' : 'top-4 right-4'} ${isMobile ? 'px-2 py-1 text-xs' : 'px-3 py-1.5 text-sm'} rounded-full font-semibold shadow-sm whitespace-nowrap z-10`}
            style={{
        ...
        <div className={`flex items-center ${isMobile ? 'mb-3' : 'mb-6'}`}>
          <div className="flex items-center flex-1 min-w-0" style={{ paddingRight: isMobile ? '80px' : '100px' }}>
            ...
            <h3 className={`${isMobile ? 'text-lg' : 'text-3xl'} font-bold truncate`} style={{ color: textColor }}>Battery #{battery.power}</h3>
          </div>
        </div>
          ...
          <div className="flex flex-col items-center flex-shrink-0">
            ...
            {!isCompact && (
              <div className="mt-4 text-center">
                <div className={`${isMobile ? 'text-xs' : 'text-base'} mb-1 whitespace-nowrap`} style={{ color: secondaryTextColor }}>State of Charge</div>
                <div className={`${isMobile ? 'text-xs' : 'text-sm'} whitespace-nowrap`} style={{ color: secondaryTextColor }}>
          ...
          <div className={`grid grid-cols-2 ${isMobile ? 'gap-2' : 'gap-3'} flex-1`}>
            <div className={`${isMobile ? 'flex flex-col items-center justify-center py-2 px-2' : 'flex justify-between items-center py-2 px-2'} rounded-xl`} style={{ backgroundColor: metricBgColor }}>
              <div className={`flex items-center ${isMobile ? 'mb-1' : 'min-w-0 flex-shrink'}`}>
                ...
                <span className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium whitespace-nowrap`} style={{ color: secondaryTextColor }}>Voltage</span>
              </div>
              <span className={`${isMobile ? 'text-sm' : 'text-base'} text-green-600 font-bold whitespace-nowrap ${isMobile ? 'mt-0.5' : 'ml-1 flex-shrink-0'}`}>
            ...
            <div className={`${isMobile ? 'flex flex-col items-center justify-center py-2 px-2' : 'flex justify-between items-center py-2 px-2'} rounded-xl`} style={{ backgroundColor: metricBgColor }}>
              <div className={`flex items-center ${isMobile ? 'mb-1' : 'min-w-0 flex-shrink'}`}>
                ...
                <span className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium whitespace-nowrap`} style={{ color: secondaryTextColor }}>Current</span>
              </div>
              <span className={`${isMobile ? 'text-sm' : 'text-base'} text-blue-600 font-bold whitespace-nowrap ${isMobile ? 'mt-0.5' : 'ml-1 flex-shrink-0'}`}>
            ...
            <div className={`${isMobile ? 'flex flex-col items-center justify-center py-2 px-2' : 'flex justify-between items-center py-2 px-2'} rounded-xl`} style={{ backgroundColor: metricBgColor }}>
              <div className={`flex items-center ${isMobile ? 'mb-1' : 'min-w-0 flex-shrink'}`}>
                ...
                <span className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium whitespace-nowrap`} style={{ color: secondaryTextColor }}>Temp</span>
              </div>
              <span className={`${isMobile ? 'text-sm' : 'text-base'} text-orange-600 font-bold whitespace-nowrap ${isMobile ? 'mt-0.5' : 'ml-1 flex-shrink-0'}`}>{battery.temperature?.toFixed(2) || '0.00'} °C</span>
            </div>
            <div className={`${isMobile ? 'flex flex-col items-center justify-center py-2 px-2' : 'flex justify-between items-center py-2 px-2'} rounded-xl`} style={{ backgroundColor: metricBgColor }}>
              <div className={`flex items-center ${isMobile ? 'mb-1' : 'min-w-0 flex-shrink'}`}>
                ...
                <span className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium whitespace-nowrap`} style={{ color: secondaryTextColor }}>Delta V</span>
              </div>
              <span className={`${isMobile ? 'text-sm' : 'text-base'} text-red-600 font-bold whitespace-nowrap ${isMobile ? 'mt-0.5' : 'ml-1 flex-shrink-0'}`}>
            ...
            <div className={`${isMobile ? 'flex flex-col items-center justify-center py-2 px-2' : 'flex justify-between items-center py-2 px-2'} rounded-xl`} style={{ backgroundColor: metricBgColor }}>
              <div className={`flex items-center ${isMobile ? 'mb-1' : 'min-w-0 flex-shrink'}`}>
                ...
                <span className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium whitespace-nowrap`} style={{ color: secondaryTextColor }}>Power</span>
              </div>
              <span className={`${isMobile ? 'text-sm' : 'text-base'} text-purple-600 font-bold whitespace-nowrap ${isMobile ? 'mt-0.5' : 'ml-1 flex-shrink-0'}`}>
          ...
          <div className="mt-10">
            ...
            >
              <div className="flex items-center">
                ...
                <h4 className={`${isMobile ? 'text-lg' : 'text-xl'} font-semibold whitespace-nowrap`} style={{ color: textColor }}>Cell Voltages</h4>
              </div>
              <ChevronDown 
                ...
                return (
                  ...
                  >
                    <div className="relative">
                      ...
                      <div 
                        className={`absolute ${isMobile ? '-top-0.5 -right-0.5' : '-top-1 -right-1'} ${isMobile ? 'w-5 h-5' : 'w-6 h-6'} rounded-full flex items-center justify-center ${isMobile ? 'text-xs' : 'text-sm'} font-bold border-2`}
                        style={{
                    ...
                    <div className={`${isMobile ? 'text-xs' : 'text-sm'} font-mono font-semibold whitespace-nowrap ${isLowVoltage ? 'text-red-600' : 'text-indigo-600'}`}>
  ...
  return (
    <div className="min-h-screen p-6" style={{ backgroundColor: pageBackgroundColor }}>
      {/* Header */}
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-4xl font-bold" style={{ color: textColor }}>Battery Details</h1>
          <button
            onClick={() => navigate(-1)}
      ...
      {allBanks.length > 1 && (
        <div className="mb-6">
          <label className="text-base font-medium mb-3 block" style={{ color: textColor }}>
            Select Battery Bank:
          </label>
          <div className="flex flex-wrap gap-3">
            {allBanks.map((b: BatteryBank) => {
                ...
                <button
                  ...
                  style={{
                    ...
                    boxShadow: isSelected 
                      ? `0px 4px 12px ${theme === 'dark' ? 'rgba(59, 130, 246, 0.3)' : 'rgba(59, 130, 246, 0.2)'}`
                      : 'none',
                  }}
                >
                  {displayName}
      ...
      <div className="space-y-6" style={{ backgroundColor: pageBackgroundColor }}>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              ...
              <h2 className={`${isMobile ? 'text-xl' : 'text-3xl'} font-bold whitespace-nowrap`} style={{ color: textColor }}>Overview</h2>
            </div>
            <div className="text-sm">
              <span 
                className={`${isMobile ? 'px-2 py-1 text-xs' : 'px-4 py-2 text-base'} rounded-full font-semibold shadow-sm whitespace-nowrap`}
          style={{
        ...
        <div className="space-y-2">
              <div className={`${isMobile ? 'text-sm' : 'text-base'} whitespace-nowrap`} style={{ color: secondaryTextColor }}>Voltage</div>
              <ProgressBar 
                value={bank.voltage} 
                max={60} 
                unit="V" 
            ...
            <div className="space-y-2">
              <div className={`${isMobile ? 'text-sm' : 'text-base'} whitespace-nowrap`} style={{ color: secondaryTextColor }}>SOC</div>
              <ProgressBar 
                value={bank.soc} 
                max={100} 
                unit="%" 
            ...
            <div className="space-y-2">
              <div className={`${isMobile ? 'text-sm' : 'text-base'} whitespace-nowrap`} style={{ color: secondaryTextColor }}>Temperature</div>
              <ProgressBar 
            ...
            <div className="space-y-2">
              <div className={`${isMobile ? 'text-sm' : 'text-base'} whitespace-nowrap`} style={{ color: secondaryTextColor }}>Power</div>
              <ProgressBar 
          ...
          <div className={`mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 ${isMobile ? 'text-sm' : 'text-base'}`}>
