...
export const NewAppLayout: React.FC = () => {
    ...
    const fetchArrays = async () => {
      try {
        ...
        if (response.arrays && response.arrays.length > 0) {
          ...
          // Don't auto-select array - let user choose home or array
          // Auto-select first array only if explicitly needed
          // if (!selectedArray) {
  ...
  return (
    <ArrayContext.Provider value={{ arrays, selectedArray, setSelectedArray }}>
      <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900">
          ...
          <>
            ...
            >
              ...
              <div className="overflow-y-auto max-h-[calc(70vh-20px)]">
                <Sidebar onClose={() => setSidebarOpen(false)} isDrawer={true} />
              </div>
            </div>
        ...
        <div className={`flex-1 flex flex-col ${isMobile ? 'w-full' : 'ml-16'}`}>
          <Outlet />
        </div>
      </div>
